...
{
    public class InMemoryItemRepositoryOffsetShould
    {
        ...
        public InMemoryItemRepositoryOffsetShould()
        {
            PopulateItems(100);
        ...
        {
            var criteria = new Criteria(new Filters(new List<Filter>()), null, null, 50);

            var result = await _repository.Matching(criteria);
            Assert.Equal(result, _items.Skip(50));
        }
    }
}